"""Arrow schemas for the otela agent-trace (at/v2) spec.
...
Sessions (`at/v2`)
------------------
- `session_id` is propagated to every span the source instrumentation tagged
  with one of the recognized session attributes (see `normalize._detect_session_id`
  for the precedence list). It also lives on `traces` so session-level
  queries don't need a join through `spans`.
- The `sessions` rollup is materialized once at end-of-load from per-session
  accumulators, analogous to `traces`. Traces with no `session_id` are
  excluded from the rollup.
- `traces.session_turn` is otela-derived (no source convention specifies it).
  It is a 0-indexed ordinal of the trace within its session, ordered by
  `start_time_unix_nano` ASC with `trace_id` lexicographic as deterministic
  tiebreak. It is scoped to the current load — across multiple loads the
  positions are not globally consistent. Null when `session_id` is null.
"""
...
TRACES_SCHEMA = pa.schema(
    [
        ...
        pa.field("trace_id", pa.string(), nullable=False),
        pa.field("session_id", pa.string()),
        pa.field("session_turn", pa.int32()),
...
# Session-level rollup. One row per distinct session_id. Like `traces`,
# materialized once at end-of-load from per-session accumulators. Traces
# whose session_id is null are excluded.
SESSIONS_SCHEMA = pa.schema(
    [
        pa.field("spec", pa.string(), nullable=False),
        pa.field("spec_version", pa.string(), nullable=False),
        pa.field("session_id", pa.string(), nullable=False),
        pa.field("trace_count", pa.int64()),
        pa.field("span_count", pa.int64()),
        pa.field("error_count", pa.int64()),
        pa.field("start_time_unix_nano", pa.int64()),
        pa.field("end_time_unix_nano", pa.int64()),
...
# Session rollup is materialized once at end-of-load.
SESSIONS_TABLE_NAME = "sessions"

# Every table the library produces.
ALL_TABLE_NAMES = (*TABLE_NAMES, TRACES_TABLE_NAME, SESSIONS_TABLE_NAME)
